  ...
  while true; do
    ...
    case "$ans" in
      ...
      "5")
        ...
        # Валидация доменов перед использованием
        if command -v validate_domain >/dev/null 2>&1; then
          ...
          while IFS= read -r domain || [ -n "$domain" ]; do
            ...
            if validate_domain "$domain"; then
              ...
              echo -e "${yellow}Предупреждение: домен может быть некорректным: $domain${plain}" >&2
              has_invalid=1
            fi
          done <<< "$domains"
          
          if [ ${#valid_domains[@]} -eq 0 ]; then
            echo -e "${red}Ошибка: не указано ни одного валидного домена${plain}"
            pause_enter
            continue
          fi
          
          if [ "$has_invalid" -eq 1 ] && [ ${#valid_domains[@]} -gt 0 ]; then
            echo -e "${yellow}Используются только валидные домены из списка${plain}"
            user_domain=$(IFS=' '; echo "${valid_domains[*]}")
          fi
        fi
...
tcp443_submenu() {
  ...
  case "$answer_bezr" in
    "" )
      ...
      ;;
    *)
      # Улучшенная валидация: проверка что это число в допустимом диапазоне
      if [[ "$answer_bezr" =~ ^[0-9]+$ ]] && [ "$answer_bezr" -ge 0 ] && [ "$answer_bezr" -le 17 ]; then
        #Отключение
